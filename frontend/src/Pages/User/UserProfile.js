...
const UserProfile = () => {
  ...
  const handlePasswordChange = (e) => {
    ...
    if (name === 'newPassword') {
      ...
      setPasswordErrors((prevErrors) => ({ ...prevErrors, newPassword: errors.join(' ') }));
    }

    // Confirm password validation
    if (name === 'confirmPassword' || (name === 'newPassword' && passwords.confirmPassword)) {
      if (value !== passwords.newPassword) {
        setPasswordErrors((prevErrors) => ({ ...prevErrors, confirmPassword: 'Passwords do not match.' }));
      } else {
        setPasswordErrors((prevErrors) => ({ ...prevErrors, confirmPassword: '' }));
      }
    }
  };
  ...
  const handleSaveChanges = async () => {
    if (emailError || passwordErrors.newPassword || passwordErrors.confirmPassword) return;
  ...
  return (
    <Box>
      <NavBar />
      <br /><br />
      <Box sx={{ padding: 4, maxWidth: 800, margin: '0 auto', border: '1px solid #ddd', borderRadius: 2, boxShadow: 3 }}>
        <Box sx={{ textAlign: 'center', marginBottom: 4 }}>
          <Avatar
            alt="Profile Picture"
            src={profile.profilePicture}
            sx={{ width: 150, height: 150, margin: '0 auto', marginBottom: 2, cursor: 'pointer' }}
            onClick={handleAvatarClick}
          />
          <Typography variant="h4" sx={{ marginBottom: 2, fontFamily: 'Times New Roman, serif' }}>{`${profile.firstName} ${profile.lastName}`}</Typography>
        </Box>
        <Box sx={{ textAlign: 'center', marginBottom: 4 }}>
          <Grid container spacing={2}>
            <Grid item xs={12}>
              <Typography variant="subtitle1">First Name: {profile.firstName}</Typography>
            </Grid>
            <Grid item xs={12}>
              <Typography variant="subtitle1">Last Name: {profile.lastName}</Typography>
            </Grid>
            <Grid item xs={12}>
              <Typography variant="subtitle1">Email: {profile.email}</Typography>
            </Grid>
            <Grid item xs={12}>
              <Typography variant="subtitle1">Address: {profile.address}</Typography>
            </Grid>
            <Grid item xs={12}>
              <Typography variant="subtitle1">Gender: {capitalizeGender(profile.gender)}</Typography>
            </Grid>
            <Grid item xs={12}>
              <Typography variant="subtitle1">Date of Birth: {formatDate(profile.dateOfBirth)}</Typography>
            </Grid>
          </Grid>
        </Box>
        ...
        <Dialog open={editProfileOpen} onClose={handleEditProfileClose}>
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12}>
                <Button
                  ...
                  sx={{ marginBottom: 2 }}
                >
                  {showPasswordFields ? 'Cancel Reset Password' : 'Reset Password'}
                </Button>
              </Grid>
              {showPasswordFields && (
                <>
          ...
          <DialogActions>
            ...
            <Button onClick={handleSaveChanges} disabled={uploading || !!emailError || !!passwordErrors.newPassword || !!passwordErrors.confirmPassword}>
              Save Changes
            </Button>
          </DialogActions>
        </Dialog>
          ...
          <DialogContent>
            ...
            <DialogActions>
              ...
              {profilePictureChanged && (
                <Button onClick={handleSaveProfilePicture} disabled={uploading}>
